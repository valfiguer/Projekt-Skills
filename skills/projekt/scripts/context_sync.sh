...
if [ "$PJ_LAST_STATUS" = "403" ] && [ -n "$PROJECT_ID" ]; then
...
elif [ "${PJ_LAST_STATUS:0:1}" != "2" ]; then
  pj_die "GET /organizations/$ORG/projects failed (HTTP $PJ_LAST_STATUS): $(echo "$PROJECTS" | jq -r '.message // .error // .detail // .' 2>/dev/null)"
else
...
if [ "${PJ_LAST_STATUS:0:1}" != "2" ]; then
  echo "  ⚠ GET /organizations/$ORG/members → HTTP $PJ_LAST_STATUS (assignee resolution limited)" >&2
  mem_slim='[]'
else
