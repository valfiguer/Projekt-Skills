...
#   api_base : $TREXA_API_BASE             > auth.json .api_base > https://api.projektrepublic.com/api/v1
...
PJ_LAST_STATUS=0   # see pj_last_status: pj_req almost always runs in a subshell
PJ_STATUS_FILE="${PJ_STATUS_FILE:-$PJ_RUN_DIR/.last_status}"

# pj_req is nearly always called as `X="$(pj_req GET /path)"`, and command
# substitution runs in a SUBSHELL — so a plain variable assignment inside it is
# lost to the caller. The status is therefore also written to a file, and callers
# read it with pj_last_status. Using $PJ_LAST_STATUS directly after a capture
# reads the initial 0 and silently misclassifies every error.
pj_last_status() { cat "$PJ_STATUS_FILE" 2>/dev/null || echo 0; }
...
# Accept a base written with or without the /v1 segment: a stored api_base of
# ".../api" 404s EVERY call, and since the stored value wins over the default the
# failure reads as a dead API rather than a typo.
_pj_norm_base() { local b="${1%/}"; case "$b" in */api) printf '%s/v1' "$b" ;; *) printf '%s' "$b" ;; esac; }

pj_api_base() {
  if [ -n "${TREXA_API_BASE:-}" ]; then _pj_norm_base "$TREXA_API_BASE"; return; fi
  if [ -f "$PJ_AUTH_FILE" ]; then
    local b; b=$(_pj_jq -r '.api_base // empty' "$PJ_AUTH_FILE")
    [ -n "$b" ] && { _pj_norm_base "$b"; return; }
  fi
  printf 'https://api.projektrepublic.com/api/v1'
}
...
pj_req() {
  ...
  local token base org url hdr_file body_file out code attempt=0 sleep_s
  ...
  while :; do
    attempt=$((attempt+1))
    # The body goes to its OWN file, never to /dev/stdout alongside -w: curl does
    # not order those two streams, so a multi-line JSON body and the status code
    # interleave. That made every error report a bogus status AND silently broke
    # the 429/5xx retry, which compares $code numerically.
    local -a args=(-sS -X "$method" -D "$hdr_file" -o "$body_file" -w '%{http_code}'
    ...
    out="$(cat "$body_file")"
    PJ_LAST_STATUS="$code"
    mkdir -p "$(dirname "$PJ_STATUS_FILE")" 2>/dev/null
    printf '%s' "$code" > "$PJ_STATUS_FILE" 2>/dev/null
