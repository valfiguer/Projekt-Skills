...
```bash
python3 skills/pr-tokens/scripts/medir.py                     # current repo, last 10 sessions
python3 skills/pr-tokens/scripts/medir.py --todas             # whole history
python3 skills/pr-tokens/scripts/medir.py --proyecto ~/dev/x  # another repo
python3 skills/pr-tokens/scripts/medir.py --breve             # three lines (what the hook prints)
python3 skills/pr-tokens/scripts/medir.py --json              # for another program
```
...
```bash
python3 skills/pr-tokens/scripts/instalar.py --repo .            # DRY-RUN, shows the diff
python3 skills/pr-tokens/scripts/instalar.py --repo . --apply
python3 skills/pr-tokens/scripts/instalar.py --repo . --quitar --apply
```

The block lives in `skills/pr-tokens/reglas-del-proyecto.md` — read it before installing.
...
```bash
python3 skills/pr-tokens/scripts/perfil_mcp.py --tarea "registrar una factura de proveedor"
python3 skills/pr-tokens/scripts/perfil_mcp.py --dominios finance,crm
python3 skills/pr-tokens/scripts/perfil_mcp.py --dominios pm --desde-contrato <path>/openapi.json
```
...
`$TMPDIR/pr-tokens-<user>/incumplimientos.log` — and lets the command through. Blocking costs
