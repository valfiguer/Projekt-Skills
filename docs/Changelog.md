...
### Changed
- **Consistent CLI across skills:** every project argument is now `--project` (accepts id / key / name), and `--apply` is always placed **after** the sub-command. Previously `pr-informes` took a positional `project` and `pr-docs` used `--project-id` (UUID-only) with a top-level `--apply`.
